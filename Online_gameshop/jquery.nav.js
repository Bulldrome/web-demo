	...
	init:function(setting){
		...
		function buildmenu($anchors){
			$anchors.each(function(){ //loop through anchor links
				...
				$submenu.wrap('<div class="megawrapper" style="z-index:'+down_menu.startzindex+';position:absolute;top:0;left:0;visibility:hidden"><div style="position:absolute;overflow:hidden;left:0;top:0;width:100%;height:100%;"></div></div>')
				...
				var $wrapper=$submenu.closest('div.megawrapper').css({width:$submenu.outerWidth()+down_menu.wrapperoffset[0], height:$submenu.outerHeight()+down_menu.wrapperoffset[1]}) //reference outermost wrapper of submenu and set its dimensions
				...
				$anchor.bind((setting.trigger=="click")? "click" : "mouseenter", function(e){ //when mouse clicks on or mouses over anchor
					...
					$wrapper.css({visibility:'visible', left:offset.left-(orienttoleft? $wrapper.outerWidth()-$anchor.outerWidth()-down_menu.wrapperoffset[0] : 0)+1, top:offset.top+$anchor.outerHeight(), zIndex:++down_menu.startzindex})
					$submenu.stop().animate({top:0}, s.dur, s.easing) //animate submenu into view
					if (setting.trigger=="click" && !down_menu.ismobile) //returning false in mobile browsers seem to lead to strange behavior
						return false
				})
				...
				$anchor.click(function(e){
					if (down_menu.ismobile) //on ipad/iphone, disable anchor link (those with a drop down menu) when clicked on (triggered by mouseover event on desktop), so menu is given a chance to appear
						return false
				})
